...
public class CScanQueue extends Queue {
    ...
    public Cylinder getNextCylinder(int currentCylinderNumber, ScanDirection currentScanDirection, boolean returningForCircularScan) {
        if (isEmpty()) {
            return null;
        }
        ...
        // 복귀중이 아니라면
            ...
            if (nextCylinder == null && !cylindersOutRange.isEmpty()) {
                ...
                for (int i = 1; i < cylindersOutRange.size(); i++) {
                    ...
                    int distance = cylinder.getDistanceFrom(currentCylinderNumber);
                    int maxDistance = nextCylinder.getDistanceFrom(currentCylinderNumber);
